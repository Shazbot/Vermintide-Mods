--- Basically just added max_depth to pretty.write to prevent stack overflows
...
function pretty.read(s)
    ...
    s = 'return '..s
    local chunk,err = utils:load('tbl','t',{})
...
function pretty.load (s, env, paranoid)
    ...
    local chunk,err = utils:load('tbl','t',env)
    if not chunk then return nil,err end
...
function pretty.write (tbl,space,not_clever, max_depth)
    ...
    local writeit
    writeit = function (t,oldindent,indent, current_depth)
        local tp = type(t)
            ...
            if not not_clever then
                for i,val in ipairs(t) do
                    if current_depth+1 < max_depth then
                        put(indent)
                        writeit(val,indent,newindent, current_depth+1)
                    end
                    used[i] = true
                end
            end
...
-- to stdout.
function pretty.dump (t, filename, max_depth)
