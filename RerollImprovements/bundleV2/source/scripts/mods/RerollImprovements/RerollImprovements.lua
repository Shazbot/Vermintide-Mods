local mod = get_mod("RerollImprovements") -- luacheck: ignore get_mod
 ...
 -- luacheck: globals UIWidget Colors UISettings
...
mod:hook_safe(CraftPageRollProperties, "_add_craft_item", function(self, backend_id)
	mod:pcall(function()
		if self._craft_items[1] and self._widgets_by_name["properties_1"] then
			local properties = Managers.backend:get_interface("items"):get_properties(self._craft_items[1])

			self._widgets_by_name["properties_1"].content.text = ""
			self._widgets_by_name["properties_2"].content.text = ""

			local index = 1
			for prop_name, prop_value in pairs( properties ) do
				self._widgets_by_name["properties_"..index].content.text = UIUtils.get_property_description(prop_name, prop_value)
				index = index + 1
			end
		end
...
mod:hook_safe(CraftPageRollProperties, "_remove_craft_item", function(self)
	if not self._craft_items[1] and self._widgets_by_name["properties_1"] then
		self._widgets_by_name["properties_1"].content.text = ""
		self._widgets_by_name["properties_2"].content.text = ""
	end
end)
...
mod:hook_safe(CraftPageRollProperties, "create_ui_elements", function(self)
	...
	for i = 1, 2 do
		local text_definition = UIWidgets.create_simple_text("", "craft_bar", nil, nil, mod.property_text_style)
		local widget = UIWidget.init(text_definition)
		widgets[#widgets + 1] = widget
		widgets_by_name["properties_"..i] = widget
		widget.offset[1] = 80 + 120
		widget.offset[2] = (i-1)*(-25) + 230
	end
end)
